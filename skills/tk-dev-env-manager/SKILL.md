...
**变更后必须验证。** 本 skill 变更任何文件（如创建/修改 .env、.description 等配置文件）后，必须重新执行环境检测验证变更是否正确，并确认是否满足用户的原始需求。
...
- [ ] 环境变量路径是否正确？
- [ ] 重新执行环境检测是否通过？
- [ ] 目标工具是否可正常执行（java -version / node --version 等）？
- [ ] 是否满足用户原始需求？

如果验证未通过，必须修复后再交付。
```

**提问后必须重新确认。** 向用户提问获取回答后，必须基于回答重新出具完整方案，并将该方案以菜单选项形式交给用户最终确认。用户选择确认选项后，方可进入下一步。未获得用户确认的方案不得执行。

**方案必须包含详细的改动信息。** 提交给用户确认的方案中，对于每个文件变更必须包含：
- **新建文件**：文件完整路径、文件名、文件类型、核心职责、一句话说明改动逻辑
- **修改文件**：文件完整路径、修改内容摘要、为何需要修改
- **删除文件**：文件完整路径、删除原因
- **配置变更**：配置项名称、原值/新值、变更原因

示例格式：
```
【文件变更清单】
新增文件：
  - .claude/project-env/.env                                  — 项目环境变量配置
  - .claude/project-env/.description                          — 环境配置说明文档

修改文件：
  - .gitignore                                               — 新增 .claude/project-env/ 排除项

配置变更：
  - .env: JAVA_HOME=C:/Users/xxx/.jdks/corretto-17           — JDK 安装路径配置
```

禁止仅用"配置环境变量"等模糊描述，必须逐项列出具体文件路径和配置内容。

**允许并行 Agent 加速。** 在 E0-E3 阶段中，当需要检测多个工具或多个项目时，允许启用最多 5 个子 agent 并行执行以加速任务：
- E0 阶段：多个子项目的类型检测可以并行
- E1 阶段：多个项目的配置文件解析可以并行
- E2 阶段：多个配置文件读取可以并行
- E3 阶段：多个工具的版本检测可以并行

子 agent 必须有明确的输入、输出和职责边界，只负责指定任务的检测/读取操作。
...
MAVEN_HOME=C:/Users/xxx/.m2/wrapper/dists/apache-maven-3.9.9

# Node 环境
NODE_HOME=C:/Users/xxx/.nvm/versions/node/v22.14.0

# Python 环境
PYTHON_HOME=C:/Users/xxx/.pyenv/versions/3.12.8
```
...
**直接向用户展示上述选项，等待用户回复选项字母（A/B/C/D）进行选择。**
